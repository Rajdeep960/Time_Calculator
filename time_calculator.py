...
def calculate_hours(h1, m1, type, h2, m2, today=""):
    ...
    if today == "":
        if day_count == 0:
            print(f"{int(h_end)}:{int(m_end):02d} {t_type}") 
        elif day_count == 1:
            print(f"{int(h_end)}:{int(m_end):02d} {t_type} (next day)")
        else:
            print(f"{int(h_end)}:{int(m_end):02d} {t_type} ({int(day_count)} days later)")
    else:
        ...
        # for printing
        if day_count == 0:
            print(f"{int(h_end)}:{int(m_end):02d} {t_type}, {day_name}") 
        elif day_count == 1:
            print(f"{int(h_end)}:{int(m_end):02d} {t_type}, {day_name} (next day)")
        else:
            print(f"{int(h_end)}:{int(m_end):02d} {t_type}, {day_name} ({int(day_count)} days later)")


def add_time(startTime, durationTime, startingDay=""):
    startingTimeDivided(startTime)
    durationTimeDivided(durationTime)
    calculate_hours(startingTime_hours, startingTime_minute, time_type, durationTime_hours, durationTime_minute, startingDay)
